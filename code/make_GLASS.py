...
def make_mock(params_dict, fiducial_cosmo = False, no_noise=False):
    ...
    redshift_distributions_sources['z'] = np.hstack([0,mu[8+random_rel].data['Z_MID']])
    for ix in [1,2,3,4]:
    ...
    f1,f2,f3,f4 = compute_mean(mu[8+int(random_rel)])

    params_dict['dz1'] = f1-r1
    params_dict['dz2'] = f2-r2
    params_dict['dz3'] = f3-r3
    params_dict['dz4'] = f4-r4
    ...
    for i, delta_i in enumerate(matter):
        ...
        for ix in [1,2,3,4]:
            z_i, dndz_i = glass.shells.restrict(redshift_distributions_sources['z'], redshift_distributions_sources['bins'][ix], ws[i])
    ...
    m_sources = [-0.002,-0.017,-0.029,-0.038]#,[-0.006,-.01,-0.026,-0.032]
    ms_sources = [0.0091,0.0078,0.0076,0.0076]
    for tomo_bin in [1,2,3,4]:
        m_ = 1+m_sources[tomo_bin-1]
        s_ = ms_sources[tomo_bin-1]
...
output_folder = '/pscratch/sd/m/mgatti/GLASS/'
def doit():
    mock_num = np.random.randint(0,100000000,1)[0]
    if not os.path.exists(output_folder+str(mock_num)+'.npy'):
        sources_cat = make_mock(draw_parameters_Dirac(), fiducial_cosmo = False, no_noise=False)
        np.save(output_folder+str(mock_num),sources_cat)
...
#srun --nodes=4 --tasks-per-node=64   python make_GLASS.py 