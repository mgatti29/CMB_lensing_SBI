...
def runit(runs_to_do,count):
    ...
    for rot in range(4):
        ...
        path = output_DV+'CMB_lensing_{0}_{1}_{2}_{3}.npy'.format(runs_to_do[count]['folder'],runs_to_do[count]['mock_num'],runs_to_do[count]['noiserel'],rot)
    ...
    for f in files:
        try:
            ...
            if (noiserel <10):
                ...
                path = output_DV+'CMB_lensing_{0}_{1}_{2}_{3}.npy'.format(folder,mock_num,noiserel,3)
...
#
        if (run_count+comm.rank)<len(runs_to_do.keys()):
            #try:
                runit2(runs_to_do,run_count+comm.rank)
            #except:
            #    pass
        #if (run_count)<len(runstodo):
        #    make_maps(runstodo[run_count])
...
#srun --nodes=4 --tasks-per-node=32   python run_moments.py